...
export default async function Module3SectionPage({ params }: PageProps) {
  const { section } = await params;
  
  // Load markdown content from Docusaurus
  const content = await loadMarkdownContent(`module3/${section}.md`);
  ...
  const allSections = await getModuleFiles('module3');
